...
module.exports = () => {
  ...
  for (let i = 0; i < cards.length; i++, cardNumGuess++) {
    ...
    if (!Array.isArray(card.fusionTypes)) {
      errors.push(makeError(i, card, 'fusionTypes is not an array'));
    }
    else {
      for (let type of card.fusionTypes) {
        if (!FUSION_TYPES.includes(type)) {
          errors.push(makeError(i, card, `unknown fusion type: "${type}"`));
        }
      }
    }
  }
